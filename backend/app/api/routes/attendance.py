...
@router.get("/trend")
async def get_attendance_trend() -> list[dict]:
...
@router.get("/summary")
async def get_all_attendance_summaries() -> list[dict]:
...
async def get_attendance(student_index: str) -> list[dict]:
    db = get_db()
...
async def get_attendance_summary(student_index: str) -> dict:
    db = get_db()
    records = await db.attendance.find({"student_index": student_index}).to_list(length=None)
