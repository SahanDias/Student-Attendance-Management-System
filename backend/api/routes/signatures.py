...
@router.get("")
async def list_signature_review(db: AsyncIOMotorDatabase = Depends(get_db)) -> list[dict]:
    records, session_by_id, student_by_index = await _fetch_review_context(db)
    if not records:
        return []
...
@router.get("/sessions")
async def list_signature_sessions(db: AsyncIOMotorDatabase = Depends(get_db)) -> list[dict]:
    records, session_by_id, _ = await _fetch_review_context(db)
    if not records:
        return []
...
async def get_signature_session_items(
...
@router.get("/reviews")
async def list_signature_reviews(db: AsyncIOMotorDatabase = Depends(get_db)) -> list[dict]:
...
async def verify_student_signatures(
...
async def submit_signature_review(
    student_index: str,
    decision: SignatureReviewDecision,
