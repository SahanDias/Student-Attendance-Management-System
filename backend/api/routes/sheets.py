...
async def _process_session(
    session_id: str, options: ProcessOptions, db: AsyncIOMotorDatabase
) -> None:
...
async def process_sheet(
    ...
    background_tasks.add_task(_process_session, session_id, resolved_options, db)
...
async def get_results(
    ...
    # AttendanceRecord itself only ever stores student_index, not the
    # student's name (see app/models/schemas.py) -- the roster in the
    # students collection is the existing source of truth for index -> name,
    # the same one GET /students and attendance.py's summary endpoint read.
    student_indexes = {record["student_index"] for record in records}
    students = await db.students.find({"index": {"$in": list(student_indexes)}}).to_list(
        length=None
    )
    name_by_index = {student["index"]: student.get("name") for student in students}

    for record in records:
        record["_id"] = str(record["_id"])
        record["name"] = name_by_index.get(record["student_index"])
    return records
